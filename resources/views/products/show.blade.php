...
<div class="container">
  ...
  <div class="pdp">
    <div class="pdp-gallery" data-reveal>
      ...
      @else
        <div class="main-img" style="font-family:var(--font-display);font-weight:800;font-size:3rem;color:var(--brand)">{{ strtoupper(substr($product->brand->name ?? 'T',0,1)) }}</div>
      @endif
    </div>
    ...
    <div data-reveal data-delay="100">
      ...
      @if($minPrice !== null)
        <div class="best-price-box">
          <div><small style="color:var(--ink-2);font-weight:600">Best price</small><br><span class="price-xl">{{ \App\Support\Currency::format($minPrice, $bestOffer->currency) }}</span></div>
          <div style="font-size:14px;color:var(--ink-2)">Available from <strong>{{ $offers->whereNotNull('current_price')->count() }}</strong> {{ $offers->whereNotNull('current_price')->count() == 1 ? 'store' : 'stores' }}
  ...
  <section class="section" id="offers" style="margin-top:12px">
    <div class="sec-head" data-reveal><h2>Compare Stores</h2></div>
    @if($offers->isEmpty())
  ...
  <section class="section" style="margin-top:24px">
    <div class="sec-head" data-reveal><h2>Key Specifications</h2></div>
    <div class="pane" style="overflow:hidden" data-reveal><table class="spec-table">
  ...
  @if($product->pros || $product->cons)
    <div class="pros-cons section" style="margin-top:24px">
      @if($product->pros)<div class="pane"><strong style="color:var(--ok)">✓ Pros</strong><ul style="padding-left:18px;margin-top:8px;line-height:1.8">@foreach($product->pros as $pro)<li>{{ $pro }}</li>@endforeach</ul></div>@endif
      @if($product->cons)<div class="pane"><strong style="color:var(--danger)">✕ Cons</strong><ul style="padding-left:18px;margin-top:8px;line-height:1.8">@foreach($product->cons as $con)<li>{{ $con }}</li>@endforeach</ul></div>@endif
    </div>
  @endif

  <section class="section" style="padding-bottom:40px;margin-top:12px">
    <div class="sec-head" data-reveal><h2>Similar Products</h2></div>
    @if($similar->isEmpty())
    ...
    @if($cheaper->isNotEmpty())
      <div class="sec-head" style="margin-top:28px"><h2>Want something cheaper?</h2></div>
    ...
    @if($relatedGuides->isNotEmpty())
      <div class="sec-head" style="margin-top:28px"><h2>Related Reading</h2></div>
