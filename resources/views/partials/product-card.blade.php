@php($best = $product->activeOffers->filter(fn($o)=>$o->status==='active'&&$o->current_price!==null)->min('current_price'))
<article class="card">
  <a href="{{ route('product.show', $product->slug) }}" class="card-img" aria-hidden="true" tabindex="-1">
    ...
    @else{{ substr($product->brand->name ?? 'T', 0, 1) }}@endif
  </a>
  <div class="card-body">
    <span class="card-brand">{{ $product->brand->name ?? '' }}</span>
    <a class="card-name" href="{{ route('product.show', $product->slug) }}">{{ $product->name }}</a>
    <div class="stores-n">From {{ $product->active_offers_count ?? $product->activeOffers->count() }} {{ ($product->active_offers_count ?? $product->activeOffers->count()) == 1 ? 'store' : 'stores' }}</div>
    @if($best !== null)
      <div class="card-price-row">
        <span class="price-now">{{ \App\Support\Currency::format((float)$best) }}</span>
        @if($drop = $product->latestDrop??null)<span class="badge badge-drop">↓ {{ round($drop->drop_percent) }}%</span>@endif
        @php($orig = $product->activeOffers->filter(fn($o)=>$o->original_price && $o->original_price>$o->current_price))
        @if($orig->isNotEmpty())
          @php($o2 = $orig->sortByDesc(fn($o)=>$o->discountPercent())->first())
    ...
    @else
      <div class="card-price-row"><span class="badge badge-out">Price unavailable</span></div>
    @endif
    ...
    <div class="card-actions">
      ...
      @if($cheap = $product->activeOffers->whereNotNull('current_price')->sortBy(fn ($o) => [$o->availability === 'in_stock' ? 0 : 1, (float) $o->current_price])->first())
        <a class="btn btn-outline btn-sm" rel="nofollow sponsored noopener"
