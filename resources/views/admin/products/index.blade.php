  ...
  <form method="POST" action="{{ route('admin.products.bulk') }}" id="bulk-form">
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead><tr><th style="width:28px"></th><th>Name</th><th>Category / Brand</th><th>Offers</th><th>Status</th><th style="width:120px"></th></tr></thead>
          ...
          <tr>
            ...
            <td>
              <div class="ad-cell-main">
                <span class="ad-thumb" style="color:var(--ink-2)">@if($img)<img src="{{ str_starts_with($img->path,'http') ? $img->path : asset('storage/'.$img->path) }}" alt="">@else{{ strtoupper(substr($p->name,0,1)) }}@endif</span>
                <span><a href="{{ route('admin.products.edit', $p) }}"><strong>{{ $p->name }}</strong></a><small>/{{ $p->slug }}</small></span>
              </div>
            </td>
            <td>{{ $p->category?->name }}<br><small style="color:var(--ink-3)">{{ $p->brand?->name ?? '—' }}</small></td>
            <td>{{ $p->offers_count }}</td>
            <td><span class="status-pill status-{{ $p->trashed() ? 'archived' : $p->status }}">{{ $p->trashed() ? 'Archived' : ucfirst(str_replace('_',' ',$p->status)) }}</span></td>
            <td>
        ...
        @empty
          <tr><td colspan="6"><div class="ad-table-empty"><b>No products found</b>Try clearing the filters, or add your first product.</div></td></tr>
        @endforelse
      </table>
    </div>
  </form>
