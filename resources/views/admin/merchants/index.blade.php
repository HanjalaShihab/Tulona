...
<div class="ad-pane">
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr><th>Name</th><th>Network</th><th>Products</th><th>Offers</th><th>Country</th><th>Last sync</th><th>Status</th><th style="width:210px"></th></tr></thead>
      @forelse($merchants as $m)
        <tr>
          <td>
            <div class="ad-cell-main">
              <span class="ad-thumb">{{ strtoupper(substr($m->name,0,1)) }}</span>
              <span><a href="{{ route('admin.merchants.edit', $m) }}"><strong>{{ $m->name }}</strong></a><small>/{{ $m->slug }}</small></span>
            </div>
          </td>
          ...
          <td>{{ $m->country ?? '—' }}</td>
          <td>{{ $m->last_synced_at?->diffForHumans() ?? 'never' }} @if($m->sync_status === 'failed')<span class="status-pill status-failed">failed</span>@endif</td>
          <td><span class="status-pill status-{{ $m->status }}">{{ ucfirst($m->status) }}</span></td>
          <td>
            <div class="ad-actions">
              <form method="POST" action="{{ route('admin.merchants.sync', $m) }}">@csrf
                <button class="btn btn-outline btn-sm">Sync now</button></form>
              @if($m->status === 'active')
              <form method="POST" action="{{ route('admin.merchants.destroy', $m) }}" onsubmit="return confirm('Disable merchant and all its offers?')">@csrf @method('DELETE')
      ...
      @empty
        <tr><td colspan="8"><div class="ad-table-empty"><b>No merchants yet</b>Add your first store to start syncing offers.</div></td></tr>
      @endforelse
    </table>
  </div>
