...
<div class="stat-cards">
  ...
  <div class="stat-card">
    ...
    @if($failedImports)
      <span class="sc-trend down">Needs attention</span>
    @else
      <span class="sc-trend up">All clear</span>
    @endif
  </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px">
      ...
      <table class="data-table">
        @forelse($topProducts as $p)
          <tr>
            ...
            <td class="num">
              <strong>{{ number_format($p->clicks_count ?? 0) }}</strong> clicks
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="2" style="text-align:center;color:var(--ink-3);padding:24px">No clicks recorded yet.</td>
          </tr>
        @endforelse
      </table>
    </div>
  ...
  <div class="panel">
    <div class="panel-head">
      ...
      <a class="btn btn-outline btn-sm" href="{{ route('admin.imports.index') }}">View all</a>
    </div>
    <div class="panel-body" style="padding:0">
      <table class="data-table">
        ...
        @empty
          <tr>
            <td colspan="2" style="text-align:center;color:var(--ink-3);padding:24px">No imports yet.</td>
          </tr>
        @endforelse
      </table>
    </div>
      ...
      <table class="data-table">
        @forelse($syncHealth as $log)
          <tr>
            ...
            <td>
              <span class="status-pill status-{{ $log->status }}">{{ ucfirst($log->status) }}</span>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="2" style="text-align:center;color:var(--ink-3);padding:24px">No sync runs yet &#8212; use &#8220;Sync now&#8221; on a merchant.</td>
          </tr>
        @endforelse
      </table>
    </div>
